...
- **Every operator DirectMLX wraps for inference is bound**: the elementwise unaries with the `scale_bias=` DirectML folds into their input, the binaries and comparisons, the activations, `reduce`, `resample`, the gathers and scatters, `top_k`, `roi_align`, the fills, and `multihead_attention`, which DirectMLX itself does not wrap. The training gradients, `ConvolutionInteger`, `QuantizedLinearConvolution` and the feature-level 6.3 `Dequantize` are left out. Signatures take tensors positionally and everything else as keywords with DirectMLX's defaults; multi-output operators return namedtuples; `FusedActivation.relu()` and its siblings replace bare `FusedActivation(OperatorType.ACTIVATION_RELU)`.
