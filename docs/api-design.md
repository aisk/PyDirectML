...
- **【已加】** `Binding.release_data()`：`Binding` 会给每个权重留一份 CPU 拷贝，但 `OWNED_BY_DML` 的数据在初始化后就归 DirectML 了，这份拷贝纯属浪费。VAE 的 320 MiB 无所谓，UNet 半精度的 5.1 GiB 不行。`device.cpp` 里原本靠 `assert` 挡住「缓冲区长度和张量对不上」的 memcpy，现在改成真的检查并抛异常 —— Release 下 `assert` 是空的，而释放过的 binding 正好会踩到它。
